...
import '../providers/store_inventory_import_mock_provider.dart';
    ...
    return Scaffold(
      ...
      floatingActionButton: accessState.status == StoreAccessStatus.ready
          ? FloatingActionButton.extended(
              ...
              label: const Text('Tạo nhập hàng'),
            )
          : null,
      body: SafeArea(
        bottom: false,
...
class _ReadyView extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // specific import permission when backend PBAC is available.
    final imports = ref.watch(storeInventoryImportMockProvider);
    ...
    return Center(
      child: ConstrainedBox(
        ...
        child: Column(
          children: [
            _InventoryImportHeader(storeId: storeId),
            const _InventoryImportTabs(),
            const _InventoryImportFilters(),
            Expanded(
              child: ListView.separated(
                ...
                itemBuilder: (context, index) {
                  return _InventoryImportCard(item: imports[index]);
                },
...
class _InventoryImportHeader extends StatelessWidget {
  ...
  const _InventoryImportHeader({required this.storeId});
      ...
      child: Row(
        children: [
          ...
          IconButton(
            ...
            onPressed: () => _showComingSoon(context, 'Tìm kiếm nhập hàng'),
          ),
          IconButton(
            tooltip: 'Tải xuống',
            icon: const Icon(Icons.file_download_outlined),
            color: AppColors.textSecondary,
            onPressed: () => _showComingSoon(context, 'Tải sổ nhập hàng'),
          ),
        ],
      ),
    );
...
class _InventoryImportTabs extends StatelessWidget {
  const _InventoryImportTabs();
...
class _InventoryImportTab extends StatelessWidget {
  final String label;
  final bool isSelected;
  final VoidCallback? onTap;

  const _InventoryImportTab({
    required this.label,
...
class _InventoryImportFilters extends StatelessWidget {
  const _InventoryImportFilters();
    ...
    return Container(
      ...
      child: Row(
        children: const [
          ...
          SizedBox(width: AppConstants.spacingSm),
          Expanded(child: _FilterButton(label: 'Trạng thái')),
        ],
      ),
    );
  }
...
class _InventoryImportCard extends StatelessWidget {
  final StoreInventoryImportMockItem item;

  const _InventoryImportCard({required this.item});
